...
def update_graph(selected_countries, y_axis, x_axis, bubble_size, bubble_y_axis, selected_year):
    ...
    fig_line = px.line(dff, x='year', y=y_axis, color='country',
                       title=f"Trend of {full_names.get(y_axis, y_axis)} over years")
    fig_bar = px.bar(dff[dff['year'] == selected_year], x='country', y=y_axis,
                     title=f"{full_names.get(y_axis, y_axis)} in {selected_year} for selected countries")
...
app.layout = html.Div([
    ...
    html.Div(children=[
        dcc.Dropdown(options=[{'label': country, 'value': country} for country in df.country.unique()],
                     multi=True,
                     value=['Canada', 'United States', 'China', 'Japan', 'Albania', 'Zimbabwe'],
                     id='dropdown-selection',
                     style={"width": '80%', "margin-left": '60px'}),

        dcc.Dropdown(
            options=[{'label': col, 'value': col} for col in df.select_dtypes(include='number').columns if
                     col != 'year'],
            value='lifeExp', id='y-axis-selection',
            style={"width": '50%', "margin-top": '4px', "margin-left": '60px'})]),

    html.Div(children=[
        dcc.Graph(id='graph-content', style={"width": '50%'}),
        dcc.Graph(id='bar-chart', style={"width": '50%'})], style={"display": "flex",
                                                                   "flex-direction": "row"}),
    ...
    html.Div(children=[
        dcc.Slider(
            ...
            max=df['year'].max(),
            value=df['year'].max(),
            marks={str(year): str(year) for year in df['year'].unique() if
                   year % 5 == 0 or year == 1950 or year == 2007},
            step=None
        )
    ], style={"width": '50%', "margin-left": '25%'}  # слайдер по годам
    ),
    ...
    html.Div(children=[
        html.Div(children=[
            ...
            dcc.Dropdown(
                ...
                value='gdpPercap', id='y-axis-bubble-selection',
        ...
        html.Div(children=[dcc.Graph(id='bubble-chart', style={"width": '100%'})], style={"width": '70%'}),
    ], style={  # пузырек
        "margin-right": '60px',
        "width": '100%',
        "display": "flex",
    ...
    html.Div(children=[
        dcc.Graph(id='top-pop-chart', style={"width": '50%'}),

        dcc.Graph(id='continent-pop-chart', style={"width": '50%'}),
    ], style={
        "width": '100%',
        "display": "flex",
        "flex-direction": "row",
